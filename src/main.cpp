...
unsigned int nStakeMinAge = 60 * 60 * 3;	// minimum age for coin age: 3HR
unsigned int nStakeMaxAge = 60 * 60 * 24 * 90;	// stake age of full weight: 90d
unsigned int nStakeTargetSpacing = 60 * 5;			// 5 minute block spacing

int64 nChainStartTime = 1548011700;
...
const string strMessageMagic = "SafeSea Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // safesea: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        // safesea: enforce minimum output amount
...
int64 CTransaction::GetMinFee(unsigned int nBlockSize, bool fAllowFree,
                              enum GetMinFee_mode mode,int nnbytes) const
{
                        ...
                        bool* pfMissingInputs)
{
    ...
    // safesea: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// safesea: find block wanted by given orphan block
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    
	{
        ...
        // SafeSea: reward for coin-year is cut in half every 64x multiply of PoS difficulty
...
// safesea: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // safesea: target change every block
    // safesea: retarget with exponential moving toward target spacing
    CBigNum bnNew;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // safesea: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // safesea: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // safesea: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // safesea: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in SafeSea
    bool fStrictPayToScriptHash = true; // Always active in SafeSea
    ...
    // safesea: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;
    if (!txdb.WriteBlockIndex(CDiskBlockIndex(pindex)))
        return error("Connect() : WriteBlockIndex for pindex failed");

    // safesea: fees are not collected by miners as in bitcoin
    // safesea: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// safesea: total coin age spent in transaction, in the unit of coin-days.
...
// safesea: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // safesea: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // safesea: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit(pindexNew->nHeight)))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // safesea: record proof-of-stake hash value
    ...
    // safesea: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // safesea: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // safesea: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock()
{
    ...
    // safesea: check that the block satisfies synchronized checkpoint
    ...
    // safesea: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // safesea: verify hash target and signature of coinstake tx
    ...
    // safesea: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // safesea: getblocks may not obtain the ancestor block rejected
    ...
    // safesea: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SafeSea", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Who lives in a pineapple under the sea? Safe Sea Food Coin!";
        CTransaction txNew;
		...
		txNew.strTxComment = "text: SafeSea genesis block";
        ...
        block.nVersion = 1;
        block.nTime    = 1548011700;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1348139;
...
printf("+++++\n");
 //        fflush(NULL);
         ...
         assert(block.hashMerkleRoot == uint256("0xf4ff920209cfbe222192c2450c3ae9ad7390dcfa98c1895f46ac51bbe4e2dc26"));
  ...
  //       block.print();
        ...
        // safesea: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint((!fTestNet ? hashGenesisBlock : hashGenesisBlockTestNet)))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // safesea: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // safesea: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // safesea: should not enter safe mode for longer invalid chain
    // safesea: if sync-checkpoint is too old do not enter safe mode
    ...
    // safesea: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // safesea: safe mode for high alert
            }
        }
    }
...
unsigned char pchMessageStart[4] = { 0xe5, 0xe6, 0xdf, 0xed };
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // safesea: record my external IP reported by peer
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // safesea: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // safesea: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    // safesea: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // safesea: simplify transaction fee - allow free = false
...
//    void *scratchbuf = scrypt_buffer_alloc();
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // safesea: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
